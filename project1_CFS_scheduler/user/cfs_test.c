...
//
// [이전 버전 실패 원인]
// 부모에서 set_nice() 후 fork()를 반복하면 fork마다 부모 vruntime 증가.
// 나중에 fork된 자식이 더 높은 vruntime을 상속받아 역전 현상 발생.
//
// [수정 방식]
// fork()를 모두 먼저 실행 → 모든 자식이 동일한 vruntime으로 시작
// 자식 내부에서 set_nice() 호출 → nice 차이만으로 스케줄링 순서 결정
//
// 검증:
//   - 4개 프로세스: nice=-3(증가량 1), nice=-1(증가량 4),
//                   nice=0(증가량 8), nice=2(증가량 32)
//   - nice=-3이 먼저, nice=2가 나중에 완료돼야 함
//   - 멀티 CPU 특성상 중간값 순서는 보장 안 될 수 있으므로
//     양 극단만 검증
...
void test_nice_scheduling_order() {
    ...
    printf("Expected: nice=-3 -1 0 2 order\n");
    ...
    // 모든 자식이 준비됐을 때 동시에 시작 신호
    // 약간 기다려서 모든 자식이 read(sfd)에서 대기하게 함
    burn_ticks(3);
    // 4개 자식에게 각각 신호
    write(sfd[1], "1", 1);
    write(sfd[1], "1", 1);
    ...
    int cnt = 0;
    int val;
    while(cnt < 4 && read(pfd[0], &val, sizeof(int)) > 0) {
        order[cnt++] = val;
    }
    close(pfd[0]);
    ...
    check(cnt == 4 && correct, "nice order: -3 -1 0 2");
...
void test_vruntime_inherit() {
    ...
    check(result[0] == 'X',
          "lower vruntime child (X) finishes before higher vruntime child (Y)");
}
...
//   - 데드락, 크래시 없이 완료되는지 확인
// [수정] 자식 내부에서 nice 설정 (vruntime 동일하게 시작)
// =============================================
void test_stability() {
    printf("\n=== Test 7: multi-process stability ===\n");
    ...
    for(int i = 0; i < n; i++) {
        int pid = fork();
        if(pid == 0) {
            set_nice(nices[i]);  // ← 자식 내부에서 nice 설정
            burn_ticks(5);
            exit(0);
        }
    }
