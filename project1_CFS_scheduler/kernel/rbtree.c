...
void rb_insert(struct rb_tree *tree, struct rb_node *z) { // node 삽입
  struct rb_node *y = 0;
...
static void _rb_transplant(struct rb_tree *tree, struct rb_node *u, struct rb_node *v) { // node 삭제
  if (u->parent == 0)
    tree->root = v;
...
struct rb_node* rb_first(struct rb_node *node) { // 최소값
  if (node == 0) return 0;
  while (node->left != 0)
    node = node->left;
  return node;
...
struct rb_node* rb_last(struct rb_node *node) { // 최대값
  if (node == 0) return 0;
  while (node->right != 0)
    node = node->right;
  return node;
